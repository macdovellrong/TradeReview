...
- [x] **数据处理**: 
    ...
    - [ ] **交易日历过滤规则说明**:
        - 分钟/小时周期可按交易日历过滤非交易时段。
        - 日/周/月周期不做交易日历过滤，避免交易日桶因边界不一致被误删。
    - [x] **高性能 K 线合成引擎**:
...
- [x] **回放功能**:
    ...
    - [x] **步进控制**: 进度条替换为前进/后退步进按钮，可选 1m~1D 步长。
- [x] **指标系统**:
    - [x] 固定指标支持：EMA (20, 30, 40, 50, 60)，颜色：红、橙、黄、绿、蓝。
    - [x] 布林带 (20, 2)，颜色：白色。
    - [x] 使用原生 PyqtGraph 绘制，解决了与 Finplot 的冲突问题。
    - [x] MACD 子图（红/淡蓝柱体 + 信号线）。
    - [x] RSI 子图（RSI6/12/24 多线 + 20/80 参考线）。
- [x] **交互优化**:
    - [x] 滚轮缩放：直接滚动缩放 X 轴，按住 **Ctrl** 滚动缩放 Y 轴。
    - [x] **测算工具**: Ctrl+左键拖动测算价差（小数点后三位）。
    - [x] **光标同步**: 十字光标同步到 MACD/RSI 子图。
